...
n_inv_filt = qcinv.opfilt_tt.alm_filter_ninv( ninv, beam, marge_monopole=True, marge_dipole=True, marge_maps=[])

chain = qcinv.multigrid.multigrid_chain( qcinv.opfilt_tt, par.chain_descr, s_cls, n_inv_filt, debug_log_prefix=(par.out_prefix + 'log_') )

soltn = np.zeros( qcinv.util.lmax2nlm(lmax), dtype=np.complex )
